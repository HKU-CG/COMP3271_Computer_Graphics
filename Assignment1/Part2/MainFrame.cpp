...
void MainFrame::LeftMouseMove(float start_x, float start_y, float curr_x, float curr_y) {
    if (modeling_state_ == OBJ_ROTATION) {
        ...
        // Hint: Convert the drag into two world-space direction vectors (screen rays), use their cross product for axis,
        //       dot product for angle, and rotate about the mesh center with T * R * T^-1.

        glm::mat4x4 transform_mat(1.f);

        // ===== SOLUTION OBJ_ROTATION BEGIN =====
        ...
        // Hint: Ray-pick to get the depth of the hit point, map both screen positions back into world space,
        //       then project out the camera-forward component so motion stays in the view plane.

        glm::mat4x4 transform_mat(1.f);

        // ===== SOLUTION OBJ_TRANSLATION BEGIN =====
        ...
        // Hint: Compute the face normal from three vertices, then find the closest point between the face-normal ray
        //       and the current mouse ray to know how far to slide along that normal.

        glm::mat4x4 transform_mat(1.f);
        int face_index = -1;

        // ===== SOLUTION OBJ_EXTRUDE BEGIN =====
        ...
        // ===== SOLUTION OBJ_SCALE BEGIN =====
        // ===== SOLUTION OBJ_SCALE END =====
