...
#define OBJ_SCALE 5                             // object local scaling
...
public:
    ...
    // Return a ray that goes through the corresponding screen point.
    // Params:
    //     scr_x: the x value in the screen space.
    //     scr_y: the y value in the screen space.
    // Return:
    //     a tuple (o, v), where (Ray = o + tv) is the ray that goes through the corresponding scree point.
...
private:
    // ===== SOLUTION HELPERS BEGIN =====
    glm::vec3 CameraRight() const;
    glm::vec3 CameraUp() const;
    glm::vec3 CameraForward() const;
    glm::vec3 ActiveMeshCenter() const;
    glm::mat4 ComposeAroundPivot(const glm::vec3& pivot, const glm::mat4& transform) const;
    // ===== SOLUTION HELPERS END =====
