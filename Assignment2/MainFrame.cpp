...
namespace {
    ...
    bool LoadTexture2D(const char *path, unsigned int &outTex) {
        int width = 0;
        int height = 0;
        int channels = 0;
...
void MainFrame::MainLoop() {
    ...
    if (!LoadTexture2D("../bunny-atlas.jpg", gDiffuseTex)) {
        std::cerr << "Fallback: diffuse texture not found, using white." << std::endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (shading_mode_ == SHADING_PHONG) {
            // Opaque Phong
            glDisable(GL_BLEND);
            DrawScene(VAO, shaderProgram, mesh_.indices_.size());
        } else {
            // Transparent (glass-like) using same fragment shader with toggle
            glEnable(GL_BLEND);
            ...
            glUniform1i(glGetUniformLocation(shaderProgram, "uTransparent"), 1);
            ...
            // Pass 1: back faces, then front faces
            glCullFace(GL_FRONT);
            glBindVertexArray(VAO);
            mesh_.Draw(VAO, shaderProgram, mesh_.indices_.size());
            // Ensure front faces blend over back faces even when depths coincide
            glCullFace(GL_BACK);
            glBindVertexArray(VAO);
            mesh_.Draw(VAO, shaderProgram, mesh_.indices_.size());
            glDepthMask(GL_TRUE);
...
void MainFrame::ProcessInput(GLFWwindow *window) {
    ...
    // Keyboard: 1=Phong, 2=Glass
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
        shading_mode_ = SHADING_PHONG;
    }
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
        shading_mode_ = SHADING_GLASS;
    }

    // Toggle front-face winding for glass rendering (F key)
    int fkey = glfwGetKey(window, GLFW_KEY_F);
    if (fkey == GLFW_PRESS && ff_key_state_ == GLFW_RELEASE) {
        front_face_cw_ = !front_face_cw_;
    }
    ff_key_state_ = fkey;
}
...
void MainFrame::DrawScene(unsigned int VAO, unsigned int shaderProgram, size_t indexCount) {
    ...
    glUniform1i(glGetUniformLocation(shaderProgram, "uTransparent"), 0);

    // Bind diffuse texture for color
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, gDiffuseTex);
    glUniform1i(glGetUniformLocation(shaderProgram, "diffuseMap"), 0);
